...
//				var segmentIndices		= polygonIndicesArray[p];
                ...
                for (int h = 1, pr = 0; h < horzSegments + 1; pr = h, h++)
                {
                    ...
                    for (int v = 0; v < vertSegments; v++)
                    {
                        subMeshVertices[v + vertSegments] = math.mul(rotation0, new float3(polygonVertices[v].x, 0, polygonVertices[v].y));
                        subMeshVertices[v               ] = math.mul(rotation1, new float3(polygonVertices[v].x, 0, polygonVertices[v].y));
                    }
        ...
        static void SplitPolygon(List<float2[]> polygons, int index)
        {
            ...
            var positivePolygon = new List<float2>();
            var negativePolygon = new List<float2>();
            for (int i = 0; i < polygon.Count; i++)
            {
                ...
                {
                    negativePolygon.Add(new float2(-kCenter, p.y));
                    positivePolygon.Add(new float2( kCenter, p.y));
                }
            }
